...
| Scrapes & aggregates textbooks into `/docs` | Local chunking & parsing via multilingual embeddings | Local vector retrieval via Chroma + Bilingual generation via Gemini/Ollama |

---

1. **Data Aggregation:** `download_korean_sources.py` automatically scrapes and pulls down selected English-Korean textbooks, lessons, and PDFs, aggregating them into the local `./docs` directory.
2. **Local Multilingual Ingestion:** Running `mini_rag.py ingest` processes files incrementally using delta detection. The text is parsed, divided into structural chunks using an overlapping sliding window, and vectorized locally using a multilingual model optimized to map both English and Korean semantics into a shared coordinate space. The resulting vectors are written directly to your local Chroma database.
3. **Hybrid Generation:** When a user queries the `mini_rag_ui.py` interface, the question is mapped against the local vector store to pull the most contextually relevant language passages. The UI bundles these multilingual source fragments with strict contextual formatting constraints, handing the payload off to Gemini 2.5 Flash to ensure highly accurate, bilingual, script-native output.

## Features

- **Local Multilingual Embeddings:** Built-in support for Korean and English text using `paraphrase-multilingual-MiniLM-L12-v2` via `sentence-transformers`.
- **Local persistent vector storage:** Powered by Chroma.
- **Incremental re-indexing:** Built with file hash tracking for delta updates.
- **Broad file support:** Handles `.md`, `.txt`, `.pdf`, and `.docx`.
- **Streamlit UI:** Clean interface for browsing retrieved passages and answers.
- **Flexible execution:** Optional Gemini or Ollama generation.

## Project Structure

- `mini_rag.py` — Core indexing, retrieval, and generation logic.
- `mini_rag_ui.py` — Streamlit UI dashboard.
- `docs/` — Downloaded source documents.
- `rag_db/` — Local Chroma vector database instances.
...
- Ollama for completely local execution.
